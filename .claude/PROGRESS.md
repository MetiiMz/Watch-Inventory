# PROGRESS — Session IV COMPLETE: 22/22 items shipped (2026-09-06/07)

## وضعیت: تکمیل شد ✔ — همه‌ی ۲۲ مورد پیاده، تست زنده و commit شدند

## کارهای این جلسه (به ترتیب commit)
1. `277eacf` → اصلاح در `3984de6` **داشبورد:** نمودار ۱۲ماه = **سه خط** — خرید (کهربایی)، فروش (آبی)، سود فروش (سبز) + نقاط هر سری و tooltip ماهانه (فروش/خرید/سود/تعداد) | باکس برندها: ارزش فروش کنار ارزش خرید | باکس ناموجودها: تصاویر ساعت.
2. `7a4ed12` **انبار/مودال فروش:** کامپوننت تقویم جلالی پاپ‌آپ (`static/js/jalali-datepicker.js` — فیلتر سال/ماه، موس، امروز/پاک) روی تاریخ خرید و فروش | قیمت فروش فقط‌نمایش؛ «قیمت نهایی» الزامی با جداکننده هزارگان هنگام تایپ | بیعانه هم قیمت نهایی | روش‌های پرداخت اختیاری/اطلاعاتی | نام+تلفن خریدار الزامی (HTML + اعتبارسنجی API) | کد فاکتور TT-YYYYMM-NNNN (`utils.invoice_code`) زیر نوع فروش.
3. `841dd1d` **تقویم:** تصویر محصول در همه‌ی مودال جزئیات (فروش/خرید/تعمیر) + همه‌ی فیلدها | رفع قیمت فروش خالی در پنل روز.
4. `daaf190` **فروش‌ها:** زیرعنوان اصلاح شد؛ کد فاکتور در مودال جزئیات؛ ویرایش تاریخ فروش، تاریخ رسید بیعانه‌ی لینک‌شده را هم‌گام می‌کند (تست زنده).
5. `28ea0fb` **تعمیرات/پیگیری:** جداکننده هزارگان هنگام تایپ — `bindMoneyInput`/`moneyIn` مشترک به app.js منتقل شد.
6. `8efd352` **پرداخت‌ها:** تصویر ساعت + رفرنس/کد دفتر/کد سایت/قیمت و تاریخ خرید روی کارت‌ها و مودال ویرایش | تاریخ تسویه: فیلد جدید `settled_at` روی Sale و Payment (migration `0004_sale_payment_settled_at`)، هنگام add-pay/تسویه کامل/ویرایش ثبت و در مودال جزئیات فروش‌ها هم نمایش داده می‌شود | فیلد انتخاب محصول در حالت ویرایش مخفی.
7. `170e266` + `f985c66` **تنظیمات:** آپلود آیکون سایت از قبل کار می‌کرد (`/api/settings/site-icon` → `Setting.site_icon`، در favicon + برند سایدبار) | نام فروشگاه حالا در عنوان تب و برند سایدبار همه‌ی صفحات می‌آید، بعد از ذخیره بدون رفرش به‌روز می‌شود | پیش‌فرض «Tick O Time» (fallback در page_ctx و API + سطر Setting با migration `0005_store_name_default`).

## تست و اعتبارسنجی نهایی
- `manage.py check` تمیز · `makemigrations --check` همگام (0001–0005 اعمال) · `node --check` روی هر ۹ فایل static/js پاس.
- curl زنده: هر ۸ صفحه 200؛ `/api/products`، `/api/calendar/day`، `/api/payments`، `/api/settings` داده سالم؛ «Tick O Time» در `<title>` و `.brand-name` رندر شد؛ fallback نام خالی هم تست شد.
- نکته‌ی shell تکرار شد: `pkill -f` داخل همان کامندِ اجرای سرور خودِ shell را می‌کشد — اجرا در subshell جدا و pkill در کامند بعدی.

---
--- (آرشیو — برنامه‌ی اولیه‌ی جلسه IV) ---
